...
public class Voronoi {
    ...
    /** the number of nodeices */
    private int nnodeices; // NOPMD
     ...
     */
    public List<GraphEdge> generateVoronoi(final ArrayList<Node> points) {
     ...
     */
    private void computeBounds(final ArrayList<Node> points) {
     ...
     */
    private void makeSortedSiteList(final ArrayList<Node> points) {

        this.nsites = points.size();
        ...
        for (int i = 0; i < this.nsites; i++) {
            this.sites[i] = new Site(points.get(i).getX(), points.get(i).getY(), i); // NOPMD SRB
        }
    ...
    private VorEdge bisect(final Site site1, final Site site2) {
        ...
        distX = site2.getX() - site1.getX();
        distY = site2.getY() - site1.getY();

        // make sure that the difference in positive
        adx = (distX > 0) ? distX : -distX;
        ady = (distY > 0) ? distY : -distY;

        temp = (site2.getX() * site2.getX() - (site1.getX() * site1.getX())) + (site2.getY() * site2.getY() - (site1.getY() * site1.getY())) * 0.5;
    ...
    private void clipLine(final VorEdge edge) {
        ...
        xPos1 = edge.reg[0].getX();
        xPos2 = edge.reg[1].getX();
        yPos1 = edge.reg[0].getY();
        yPos2 = edge.reg[1].getY();
        ...
        if (edge.aParam == 1.0) {
            ...
            if ((site1 != null) && (site1.getY() > pymin)) {
                yPos1 = site1.getY();
            }
            ...
            if ((site2 != null) && (site2.getY() < pymax)) {
                yPos2 = site2.getY();
            }
            ...
            if ((site1 != null) && (site1.getX() > pxmin)) {
                xPos1 = site1.getX();
            }
            ...
            if ((site2 != null) && (site2.getX() < pxmax)) {
                xPos2 = site2.getX();
            }
    ...
    private double dist(final Site site1, final Site site2) {
        ...
        distX = site1.getX() - site2.getX();
        distY = site1.getY() - site2.getY();
    ...
    private Site intersect(final HalfEdge el1, final HalfEdge el2) {
        ...
        if (((edge1.reg[1].getY()) < edge2.reg[1].getY())
                || ((edge1.reg[1].getY()) == edge2.reg[1].getY())
                    && (edge1.reg[1].getX() < edge2.reg[1].getX())) {
            edge = el1;
            edge3 = edge1;
        } else {
            edge = el2;
        ...
        right_of_site = xint >= edge3.reg[1].getX();
    ...
    private void voronoiBuild() {
        ...
        for (;;) {
            ...
            // intersection
            if ((newsite != null)
                    && (this.pq.empty() || (newsite.getY()) < newintstar.getY())
                        || ((newsite.getY() == newintstar.getY())
                            && (newsite.getX() < newintstar.getX()))) {
                ...
                // remove the left edge's nodeex, and put in the new one
                ...
                if (p != null) {
                    ...
                    // push the HE into the ordered linked list of nodeices
                ...
                // get the nodeex that caused this event set the nodeex number
                // - couldn't do this searlier since we didn't know when it
                // would be processed
                v.sitenbr = this.nnodeices;
                this.nnodeices += 1;
                ...
                // remove all nodeex events to do with the right HE
    ...
    /**
     * Given a list of nodeices and a list of voronoi edges, computes a set of polygons, each
     * representing a Voronoi region, then finds the nodeex in that region and constructs a map
     * from nodeex to region.
     *
     * @param   nodeices  the list of nodeices
     * @param   edges     the list of Voronoi edges
     * @return  the constructed map
     */
    public Map<Node, Path2D> makeVoronoiPolygons(final Node[] nodeices,
        final List<GraphEdge> edges) {

        Map<Node, Path2D> map;

        map = new HashMap<Node, Path2D>(edges.size());

        // Scan through all edges
        for (int onEdge = 0; onEdge < edges.size(); onEdge++) {
            findPolygon(onEdge, nodeices, edges, map, true);
            findPolygon(onEdge, nodeices, edges, map, false);
        }
     ...
     * polygon,then adds that polygon to the map based on the nodeex it contains.
     *
     * @param  onEdge    the edge
     * @param  nodeices  the list of nodeices
     * @param  edges     the list of edges
     * @param  map       the map from nodeex to containing polygon
     ...
     */
    private void findPolygon(final int onEdge, final Node[] nodeices,
        final List<GraphEdge> edges, final Map<Node, Path2D> map, final boolean isLeft) {
        ...
        start = new Node(root.xPos1, root.yPos1);
        list.add(start);
        end = new Node(root.xPos2, root.yPos2);
        list.add(end);
...
outer:
        for (;;) {
            ...
            // edge
            maxAngle = 0;
            dx1 = current.getX() - prior.getX();
            dy1 = current.getY() - prior.getY();
            found = false;
            ...
            for (GraphEdge test : edges) {
                ...
                if ((Math.abs(test.xPos1 - current.getX()) < this.epsilon)
                        && (Math.abs(test.yPos1 - current.getY()) < this.epsilon)) {

                    // Found a link
                    if ((Math.abs(test.xPos2 - start.getX()) < this.epsilon)
                            && (Math.abs(test.yPos2 - start.getY()) < this.epsilon)) {

                        // Edge closed on the starting nodeex - finished
                        finished = true;
                    ...
                    if (isLeft
                            == isCcw(prior.getX(), prior.getY(), current.getX(), current.getY(),
                ...
                } else if ((Math.abs(test.xPos2 - current.getX()) < this.epsilon)
                        && (Math.abs(test.yPos2 - current.getY()) < this.epsilon)) {

                    // Found a link
                    if ((Math.abs(test.xPos1 - start.getX()) < this.epsilon)
                            && (Math.abs(test.yPos1 - start.getY()) < this.epsilon)) {
            ...
            if (found) {
                prior = current;
                current = new Node(bestX, bestY); // NOPMD SRB
                list.add(current);
                terminal = best;
            ...
            for (;;) {
                ...
                for (GraphEdge test : edges) {
                    ...
                    if ((Math.abs(test.xPos1 - current.getX()) < this.epsilon)
                        ...
                        // Found a link
                        if ((Math.abs(test.xPos2 - end.getX()) < this.epsilon)
                                && (Math.abs(test.yPos2 - end.getY()) < this.epsilon)) {
                        ...
                        if (isLeft
                                == isCcw(current.getX(), current.getY(), prior.getX(), prior.getY(),
                                    test.xPos2, test.yPos2)) {
                        ...
                        // Found a link
                        if ((Math.abs(test.xPos1 - end.getX()) < this.epsilon)
                                && (Math.abs(test.yPos1 - end.getY()) < this.epsilon)) {
        ...
        if (finished) {
            ...
            if (len > 2) {
                ...
                for (int i = 1; i < len; i++) {
                    node1 = list.get(i - 1);
                    node2 = list.get(i);
                    line = new Line2D.Double(node1.getX(), node1.getY(), node2.getX(), node2.getY()); // NOPMD SRB
                    poly.append(line, true);
                }

                for (Node v : nodeices) {

                    if (poly.contains(v.getX(), v.getY())) {
                        map.put(v, poly);
                    }
                }
            }
